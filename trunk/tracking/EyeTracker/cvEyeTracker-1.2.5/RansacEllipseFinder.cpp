...
RansacEllipseFinder::RansacEllipseFinder()
{
	this->start_point.x = -1;
	this->start_point.y = -1;
	this->angle_step = 20;
 	this->pupil_edge_thres = 20;
	this->pupil_param[0] = 0;
	this->pupil_param[1] = 0;
	this->pupil_param[2] = 0;
	this->pupil_param[3] = 0;
	this->pupil_param[4] = 0;
}
...
void RansacEllipseFinder::starburst_pupil_contour_detection(UINT8* pupil_image, int width, int height, int edge_thresh, int N, int minimum_cadidate_features)
{
  int dis = 7;
  ...
  double angle_normal;
  double cx = this->start_point.x;
  double cy = this->start_point.y;
  int first_ep_num;
  ...
  while (edge_thresh > 5 && loop_count <= 10) {
    ...
    for (int i = 0; i < first_ep_num; i++) {
      ...
      new_angle_step = angle_step*(edge_thresh*1.0/this->edge_intensity_diff.at(i));
...
void RansacEllipseFinder::locate_edge_points(UINT8* image, int width, int height, double cx, double cy, int dis, double angle_step, double angle_normal, double angle_spread, int edge_thresh)
{
  double angle;
  stuDPoint p, *edge;
  ...
  for (angle = angle_normal-angle_spread/2+0.0001; angle < angle_normal+angle_spread/2; angle += angle_step) {
    ...
    while (1) {
      ...
      if (pixel_value2 - pixel_value1 > this->pupil_edge_thres) {
...
stuDPoint RansacEllipseFinder::get_edge_mean()
{
  stuDPoint *edge;
  int i;
...
void RansacEllipseFinder::destroy_edge_point()
{
...
void RansacEllipseFinder::get_5_random_num(int max_num, int* rand_num)
{
  int rand_index = 0;
  int r;
  int i;
...
bool RansacEllipseFinder::solve_ellipse(double* conic_param, double* ellipse_param)
{
...
stuDPoint* RansacEllipseFinder::normalize_point_set(stuDPoint* point_set, double &dis_scale, stuDPoint &nor_center, int num)
{
  double sumx = 0, sumy = 0;
  double sumdis = 0;
...
stuDPoint* RansacEllipseFinder::normalize_edge_point(double &dis_scale, stuDPoint &nor_center, int ep_num)
{
  double sumx = 0, sumy = 0;
  double sumdis = 0;
  stuDPoint *edge;
...
void RansacEllipseFinder::denormalize_ellipse_param(double* par, double* normailized_par, double dis_scale, stuDPoint nor_center)
{
...
int* RansacEllipseFinder::pupil_fitting_inliers(UINT8* pupil_image, int width, int height,  int &return_max_inliers_num)
{
  int i;
